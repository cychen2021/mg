...
public class EPCC4Checker extends Checker {
    public EPCC4Checker(Map<String, Pair<Boolean, Formula>> constraints) {
        super(new ECCBuilder(), new EPCC4Evaluator(new ECCEvaluator()), new XPCC1Generator(), constraints);
    ...
    protected Map<String, Pair<Boolean, Link>> checkAddChange(AddChange addChange) {
        ...
        for (String constraintID: related) {
            ...
            ((EPCC4Evaluator) evaluator).setChrono(ts);
            ((EPCC4Evaluator) evaluator).setChange(addChange);
            boolean truthValue;
            if (!alreadIn) {
                truthValue = ((EPCC4Evaluator) evaluator).totalEvaluate(cct, new Binding());
            }
            else {
                truthValue = evaluator.evaluate(cct, new Binding());
                ((EPCC4Evaluator) evaluator).reset();
            }
...
//            updateMeasure(constraintID, cct);
    ...
    protected Map<String, Pair<Boolean, Link>> checkDelChange(DelChange delChange) {
        ...
        for (String constraintID: related) {
            ...
            ((EPCC4Evaluator) evaluator).setChrono(ts);
            ((EPCC4Evaluator) evaluator).setChange(delChange);
            boolean truthValue;
            if (!alreadIn) {
                truthValue = ((EPCC4Evaluator) evaluator).totalEvaluate(cct, new Binding());
            }
