...
public class XPCC3Evaluator extends Evaluator {

    protected void record(CCT cct) {
        assert cct.isAffected();
        if (cct.isToBeTotallyRecomputed()) {
            return;
        }

        if (!(cct instanceof UniversalCCT || cct instanceof ExistentialCCT)) {
            cct.setToBeTotallyRecomputed();
            return;
        }

        if (!currentChange.getTargetSet().equals(((QuantifiedFormula) cct.getFormula()).getUniverse().getId())) {
            cct.setToBeTotallyRecomputed();
            ((QuantifiedCCT) cct).clearNewlyAdded();
            return;
        }

        QuantifiedCCT qcct = (QuantifiedCCT) cct;
        Context c = currentChange.getContext();

        if (currentChange instanceof Checker.AddChange) {
            qcct.record(c);
        }
        else {
            if (!qcct.obliterate(c)) {
                qcct.setToBeTotallyRecomputed();
            }
        }
    }
